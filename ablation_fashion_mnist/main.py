...
parser.add_argument('-lgpu', default = 8, type = int, help = "Number of GPU on this server")
...
parser.add_argument('-pixel', default = 28, type = int, help = 'Pixels')
...
parser.add_argument('-fargs', default = '''{"dataset":"fmnist"}''', type = str, help = "Feature extractor arguments")
...
def get_data(b, flip):
	if (b + 1) * block_size <= N_train:
		return image.featurize(b * block_size, (b + 1) * block_size, train = True, flip = flip)
	else:
		return image.featurize(b * block_size - N_train, (b + 1) * block_size - N_train, train = False, flip = flip)
		...
		if num % N_GPU == args.gpu:
			...
			for i in range(block_size):
				for j in range(block_size):
					K[:, :, :, num / N_GPU, i, j] = tangent.xz(X_i[i], X_j[j], D[bi * block_size + i], D[bj * block_size + j]).get()
					if config.flip:
						K_flip[:, :, :, num / N_GPU, i, j] = tangent.xz(X_flip_i[i], X_j[j], D_flip[bi * block_size + i], D[bj * block_size + j]).get()

if args.timing == "yes":
	t = time.time() - starting_time
	print "Time: ", t
	print "Estimate GPU hours: ", t * (70000.0 / block_size * (70000.0 / block_size + 1) / 2) / N / 3600.0
