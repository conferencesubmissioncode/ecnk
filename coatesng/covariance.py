...
void integrate(const float s[p - 2][p - 2][p - 2][p - 2], float t[ker_num][d_num][c_num][total_diag][total_diag], const float Dx[p - 2][p - 2], const float Dz[p - 2][p - 2])
{
	...
	__shared__ float a[2][array_size][array_size], b[4][array_size][array_size];
	...
	for (i = 0; i <= d; ++ i) {
        ...
        a[1][x2][y2] = a[0][x2][y2] * (ONE - acosf(fmaxf(fminf(S, ONE), NEG_ONE)) * INV_PI) + sqrtf(ONE - fminf(S * S, ONE)) * INV_PI * P;
        ...
        if ((d - i) % d_delta == 0 && i >= d_min)
            ...
            for (j = c; j >= c_min; j -= c_delta) {
                ...
                if (x2 == 1 && y2 == 1) {
                    ...
                    t[0][(d - i) / d_delta][(c - j) / c_delta][blockIdx.x][blockIdx.y] = a[0][1][1];
