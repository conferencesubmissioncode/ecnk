...
parser.add_argument('-c_delta', default = 1000, type = int, help = 'Crop pixels interval')
...
parser.add_argument('-pixel', default = 32, type = int, help = 'Pixels')
...
parser.add_argument('-fargs', required = True, type = str, help = "Feature extractor arguments")
		...
		if num % N_GPU == args.gpu:
			...
			for i in range(block_size):
				for j in range(block_size):
					K[:, :, :, num / N_GPU, i, j] = convnet.xz(X_i[i], X_j[j], D[bi * block_size + i], D[bj * block_size + j]).get()
					if config.flip:
						K_flip[:, :, :, num / N_GPU, i, j] = convnet.xz(X_flip_i[i], X_j[j], D_flip[bi * block_size + i], D[bj * block_size + j]).get()
